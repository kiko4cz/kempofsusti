...
export default function Contact() {
    ...
    const [content, setContent] = useState({
        section_title: 'Napište nám',
        description: 'Máte dotaz? Chcete přihlásit dítě? Jsme tu pro vás.',
        email: 'kempofsusti@seznam.cz',
        phone: '603 985 226',
        address: 'Areál TJ Vaňov, Brzákova 146/1, Ústí nad Labem',
        map_url: 'https://maps.google.com/maps?q=Brzákova+146/1,+Ústí+nad+Labem&t=&z=15&ie=UTF8&iwloc=&output=embed',
    });

    useEffect(() => {
        if (rawContent) {
            const section = rawContent.find(s => s.sectionId === 'contact');
            if (section) {
                        ...
                        <div className="space-y-6">
                            ...
                            >
                                ...
                                <div>
                                    ...
                                    <p className="text-xl md:text-2xl font-bold text-gray-900 group-hover:text-primary transition-colors">{content.email}</p>
                                </div>
                                {copied ? (
                            ...
                            <a href={`tel:${content.phone.replace(/\s+/g, '')}`} className="flex items-center gap-6 p-6 rounded-3xl bg-gray-50 border border-transparent hover:bg-white hover:shadow-xl transition-all duration-300 hover:border-gray-100 group cursor-pointer">
                                ...
                                <div>
                                    ...
                                    <p className="text-xl md:text-2xl font-bold text-gray-900 group-hover:text-primary transition-colors">{content.phone}</p>
                                </div>
                                <ArrowRight className="ml-auto text-gray-300 group-hover:text-primary group-hover:translate-x-1 transition-all" />
                            </a>

                            <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(content.address)}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-6 p-6 rounded-3xl bg-gray-50 border border-transparent hover:bg-white hover:shadow-xl transition-all duration-300 hover:border-gray-100 group cursor-pointer">
                                <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center text-primary shadow-sm group-hover:scale-110 group-hover:bg-primary group-hover:text-white transition-all duration-300">
                                    <MapPin size={32} />
                                </div>
                                <div>
                                    <p className="text-sm text-gray-400 font-bold uppercase tracking-wider mb-1">Místo konání</p>
                                    <p className="text-xl font-bold text-gray-900 group-hover:text-primary transition-colors">{content.address}</p>
                                </div>
                                <ArrowRight className="ml-auto text-gray-300 group-hover:text-primary group-hover:translate-x-1 transition-all" />
                            </a>
                        </div>
